...
export default class QueueMoveAction extends Action {
    ...
    validateInteractionArgs(args: [Room, boolean, string]): [boolean, string] {
        ...
        if (args[1] === false) {
            const disabledStatusEffects = this.player.getStatusEffectsDisablingCommand("move");
        ...
        if (args[1] === true) {
            const disabledStatusEffects = this.player.getStatusEffectsDisablingCommand("run");
            if (disabledStatusEffects.length > 0)
                throw new Error(errorMessageGenerator.generateCommandDisabledError(disabledStatusEffects[0]));
        }
