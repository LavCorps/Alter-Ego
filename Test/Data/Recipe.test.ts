...
describe('Recipe test', () => {
	...
	describe('test ingredientsMatch', () => {
		test('ingredientsMatch on canteen BURNER 1', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'POT OF RICE, DICED ONIONS, BUTTER, COOKING SHERRY, SHREDDED CHEESE, CHICKEN BROTH, PAN')[0];
			const burner = testGame.entityFinder.getFixture('BURNER 1', 'canteen');
		...
		test('ingredientsMatch on canteen BURNER 2', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'POT OF RICE, DICED ONIONS, BUTTER, COOKING SHERRY, SHREDDED CHEESE, CHICKEN BROTH, PAN')[0];
			const burner = testGame.entityFinder.getFixture('BURNER 2', 'canteen');
			const burnerRecipe = burner.findRecipe();
		...
		test('ingredientsMatch on canteen BURNER 3', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'POT OF RICE, DICED ONIONS, BUTTER, COOKING SHERRY, SHREDDED CHEESE, CHICKEN BROTH, PAN')[0];
			const burner = testGame.entityFinder.getFixture('BURNER 3', 'canteen');
			const burnerRecipe = burner.findRecipe();
			expect(burnerRecipe.recipe.row).toBe(recipe.row);
			const items = CollatedItem.collate(burner.getContainedItems());
			expect(burnerRecipe.ingredients.toString()).toBe(items.toString());
		...
		test('ingredientsMatch on canteen BURNER 4', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'POT OF RICE, DICED ONIONS, BUTTER, COOKING SHERRY, SHREDDED CHEESE, CHICKEN BROTH, PAN')[0];
			const burner = testGame.entityFinder.getFixture('BURNER 4', 'canteen');
			const burnerRecipe = burner.findRecipe();
			expect(burnerRecipe.recipe.row).toBe(recipe.row);
			const items = CollatedItem.collate(burner.getContainedItems());
			expect(burnerRecipe.ingredients.toString()).toBe(items.toString());
		...
		test('ingredientsMatch on video room BURNER 1', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'POT FILLED WITH WATER, SPAGHETTI NOODLES', 'POT OF SPAGHETTI')[0];
			const burner = testGame.entityFinder.getFixture('BURNER 1', 'video-room');
			const burnerRecipe = burner.findRecipe();
		...
		test('ingredientsMatch on video room BURNER 2', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'POT FILLED WITH WATER, SPAGHETTI NOODLES', 'POT OF SPAGHETTI')[0];
			const burner = testGame.entityFinder.getFixture('BURNER 2', 'video-room');
			const burnerRecipe = burner.findRecipe();
			expect(burnerRecipe.recipe.row).toBe(recipe.row);
			const items = CollatedItem.collate(burner.getContainedItems());
			expect(burnerRecipe.ingredients.toString()).toBe(items.toString());
		...
		test('ingredientsMatch on video-room BLENDER 2', () => {
			const blender = testGame.entityFinder.getFixture('BLENDER 2', 'video-room');
		...
		test('ingredientsMatch on video-room BLENDER 3', () => {
			const blender = testGame.entityFinder.getFixture('BLENDER 3', 'video-room');
			const recipe = blender.findRecipe().recipe;
			const items = CollatedItem.collate(blender.getContainedItems());
			expect(recipe.ingredientsMatch(items)).toBe(true);
		});
		...
		test('ingredientsMatch on video-room BLENDER 4', () => {
			const blender = testGame.entityFinder.getFixture('BLENDER 4', 'video-room');
			const recipe = blender.findRecipe().recipe;
			const items = CollatedItem.collate(blender.getContainedItems());
			expect(recipe.ingredientsMatch(items)).toBe(true);
		});
		...
		test('ingredientsMatch on video-room BLENDER 5', () => {
			const blender = testGame.entityFinder.getFixture('BLENDER 5', 'video-room');
			const recipe = testGame.entityFinder.getRecipes('processing', 'blender', 'BLENDER CUP OF MILK')[0];
			const items = CollatedItem.collate(blender.getContainedItems());
			expect(recipe.ingredientsMatch(items)).toBe(false);
		});

		test('ingredientsMatch on video-room BLENDER 6', async () => {
			const blender = testGame.entityFinder.getFixture('BLENDER 6', 'video-room');
			const recipe = testGame.entityFinder.getRecipes('processing', 'blender', 'BANANA MILKSHAKE')[0];
			const items = CollatedItem.collate(blender.getContainedItems());
			expect(recipe.ingredientsMatch(items)).toBe(false);
		});
	});
	...
	describe('test getSatisfactoryProcessCount', () => {
		...
		test('getSatisfactoryProcessCount in video-room BLENDER 1', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'blender', 'ORANGE', 'ORANGE SMOOTHIE')[0];
			const blender = testGame.entityFinder.getFixture('BLENDER 1', 'video-room');
			const items = CollatedItem.collate(blender.getContainedItems());
		...
		test('getSatisfactoryProcessCount in video-room SINK 1', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'small water source', 'DIRTY PLATE, DETERGENT', 'CLEAN PLATE, DETERGENT')[0];
			const sink = testGame.entityFinder.getFixture('SINK 1', 'video-room');
		...
		test('getSatisfactoryProcessCount in video-room SINK 2', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'small water source', 'DIRTY PLATE, DETERGENT', 'CLEAN PLATE, DETERGENT')[0];
			const sink = testGame.entityFinder.getFixture('SINK 2', 'video-room');
			const items = CollatedItem.collate(sink.getContainedItems());
        ...
        test('getSatisfactoryProcessCount in video-room SINK 3', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'small water source', 'DIRTY POT, DETERGENT', 'POT, DETERGENT')[0];
			const sink = testGame.entityFinder.getFixture('SINK 3', 'video-room');
			const items = CollatedItem.collate(sink.getContainedItems());
			expect(recipe.getSatisfactoryProcessCount(items)).toBe(0);
			const sinkRecipe = sink.findRecipe();
        ...
        test('getSatisfactoryProcessCount on video-room BURNER 3', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'FROZEN STEAK, FRYING PAN', 'COOKED STEAK, DIRTY PAN')[0];
			const burner = testGame.entityFinder.getFixture('BURNER 3', 'video-room');
			const items = CollatedItem.collate(burner.getContainedItems());
			expect(recipe.getSatisfactoryProcessCount(items)).toBe(0);
			const burnerRecipe = burner.findRecipe();
			expect(burnerRecipe.recipe.row).toBe(recipe.row);
        ...
        test('getSatisfactoryProcessCount on video-room BURNER 4', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'FROZEN STEAK, FRYING PAN', 'COOKED STEAK, DIRTY PAN')[0];
			const burner = testGame.entityFinder.getFixture('BURNER 4', 'video-room');
			const items = CollatedItem.collate(burner.getContainedItems());
			expect(recipe.getSatisfactoryProcessCount(items)).toBe(0);
			const burnerRecipe = burner.findRecipe();
			expect(burnerRecipe.recipe.row).toBe(recipe.row);
        ...
        test('getSatisfactoryProcessCount on video-room BURNER 5', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'FROZEN STEAK, FRYING PAN', 'COOKED STEAK, DIRTY PAN')[0];
			const burner = testGame.entityFinder.getFixture('BURNER 5', 'video-room');
			const items = CollatedItem.collate(burner.getContainedItems());
			expect(recipe.getSatisfactoryProcessCount(items)).toBe(0);
			const burnerRecipe = burner.findRecipe();
			expect(burnerRecipe.recipe.row).toBe(recipe.row);
        ...
        test('getSatisfactoryProcessCount on video-room BURNER 6', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'FROZEN CHICKEN BREAST, FRYING PAN', 'COOKED CHICKEN BREAST, DIRTY PAN')[0];
			const burner = testGame.entityFinder.getFixture('BURNER 6', 'video-room');
			const items = CollatedItem.collate(burner.getContainedItems());
			expect(recipe.getSatisfactoryProcessCount(items)).toBe(0);
			const burnerRecipe = burner.findRecipe();
			expect(burnerRecipe.recipe.row).toBe(recipe.row);
        ...
        test('getSatisfactoryProcessCount on video-room BURNER 7', () => {
            const recipe = testGame.entityFinder.getRecipes('processing', 'stovetop', 'FRYING PAN, CLEAN GLASS', 'DIRTY PAN, DIRTY GLASS')[0];
            const burner = testGame.entityFinder.getFixture('BURNER 7', 'video-room');
            const items = CollatedItem.collate(burner.getContainedItems());
            expect(recipe.getSatisfactoryProcessCount(items)).toBe(1);
            const burnerRecipe = burner.findRecipe();
            expect(burnerRecipe.recipe.row).toBe(recipe.row);
		...
		test('getSatisfactoryProcessCount on video-room CUTTING BOARD 1', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'cutting board', 'ORANGE, LARGE KNIFE', 'PEELED ORANGE, LARGE KNIFE')[0];
			const cuttingBoard = testGame.entityFinder.getFixture('CUTTING BOARD 1', 'video-room');
		...
		test('getSatisfactoryProcessCount on video-room CUTTING BOARD 2', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'cutting board', 'ORANGE, VEGETABLE PEELER', 'PEELED ORANGE, VEGETABLE PEELER')[0];
			const cuttingBoard = testGame.entityFinder.getFixture('CUTTING BOARD 2', 'video-room');
			const items = CollatedItem.collate(cuttingBoard.getContainedItems());
			expect(recipe.getSatisfactoryProcessCount(items)).toBe(10);
			const cuttingBoardRecipe = cuttingBoard.findRecipe();
			expect(cuttingBoardRecipe.recipe.row).toBe(recipe.row);
		...
		test('getSatisfactoryProcessCount on video-room CUTTING BOARD 3', () => {
			const recipe = testGame.entityFinder.getRecipes('processing', 'cutting board', 'LEATHER SKIN, BUTTON', 'CRUDE LEATHER JACKET')[0];
			const cuttingBoard = testGame.entityFinder.getFixture('CUTTING BOARD 3', 'video-room');
			const items = CollatedItem.collate(cuttingBoard.getContainedItems());
			expect(recipe.getSatisfactoryProcessCount(items)).toBe(1);
			const cuttingBoardRecipe = cuttingBoard.findRecipe();
			expect(cuttingBoardRecipe.recipe.row).toBe(recipe.row);
