...
describe('RoomItem test', () => {
    ...
    describe('decreaseUses test', () => {
        test('procedural selections are preserved when transformed into next stage', () => {
            const item = testGame.entityFinder.getRoomItem('CAPSULE', 'video-room', 'Fixture', 'FLOOR');
            ...
            item.decreaseUses();
            const newItem = testGame.entityFinder.getRoomItems('DEPLETED CAPSULE', 'video-room', true, 'Fixture', 'Floor')[1];
