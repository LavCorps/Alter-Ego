...
describe('InventoryItem test', () => {
    ...
    describe('decreaseUses test', () => {
        test('procedural selections are preserved when transformed into next stage 1', () => {
            const item = testGame.entityFinder.getInventoryItem('CAPSULE', 'Nero', undefined, 'RIGHT HAND');
            ...
            item.decreaseUses();
            const newItem = testGame.entityFinder.getInventoryItem('DEPLETED CAPSULE', 'Nero', undefined, 'RIGHT HAND');
        ...
        test('procedural selections are preserved when transformed into next stage 2', () => {
            const item = testGame.entityFinder.getInventoryItem('CAPSULE', 'Nero', undefined, 'LEFT HAND');
            const expectedSelections = new Map([
                ['species', 'lavazard'],
                ['finish', 'glass']
            ]);
            ...
            item.decreaseUses();
            const newItem = testGame.entityFinder.getInventoryItem('DEPLETED CAPSULE', 'Nero', undefined, 'LEFT HAND');
            expect(newItem.proceduralSelections).toEqual(expectedSelections);
            expect(newItem.description.text).toEqual(expectedDescription);
        });
    });
