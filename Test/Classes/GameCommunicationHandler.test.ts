...
describe('GameCommunicationHandler test', () => {
	...
	beforeAll(async () => {
        ...
        spectateChannelIds = testGame.livingPlayers.filter(player => player.spectateChannel !== null).map(player => player.spectateChannel.id);
    });
	...
	test('cached actions contain mirrored channels after mirrorAnnouncement', () => {
		...
		const dialog = new Dialog(testGame, message, player, player.location, undefined, true);
		const action = new AnnounceAction(testGame, message, player, player.location, false);
		for (const livingPlayer of testGame.livingPlayers.values())
			testGame.communicationHandler.mirrorDialogInSpectateChannel(livingPlayer, action, dialog);
	...
	test('does not exceed action cache limit and evicts oldest actions', () => {
		...
		for (let i = 0; i < actionCacheLimit; i++) {
			actions.push(new AnnounceAction(testGame, message, player, player.location, false));
			const dialog = new Dialog(testGame, message, player, player.location, undefined, true);
			for (const livingPlayer of testGame.livingPlayers.values())
				testGame.communicationHandler.mirrorDialogInSpectateChannel(livingPlayer, actions[i], dialog);
		}
		...
		let dialog = new Dialog(testGame, message, player, player.location, undefined, true);
		for (const livingPlayer of testGame.livingPlayers.values())
			testGame.communicationHandler.mirrorDialogInSpectateChannel(livingPlayer, actions[0], dialog);
		expect(sendWebhookSpectateMessageSpy).toHaveBeenCalledTimes(actionCacheLimit * spectateChannelIds.length);
        ...
        expect(testGame.communicationHandler.getActionCache().size).toBe(actionCacheLimit);
		...
		dialog = new Dialog(testGame, message, player, player.location, undefined, true);
		for (const livingPlayer of testGame.livingPlayers.values())
			testGame.communicationHandler.mirrorDialogInSpectateChannel(livingPlayer, actions[0], dialog);
		expect(sendWebhookSpectateMessageSpy).toHaveBeenCalledTimes((actionCacheLimit + 5) * spectateChannelIds.length);
        expect(testGame.communicationHandler.getActionCache().size).toBe(actionCacheLimit);
