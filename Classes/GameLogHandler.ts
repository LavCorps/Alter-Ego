...
export default class GameLogHandler {
     ...
     * @param playerList - A list of players who performed the action.
     * @param forced - Whether or not the player was forced to perform the action.
     */
    logMove(isRunning: boolean, destination: Room, playerList: string, forced: boolean) {
        const verb = isRunning ? `ran` : `moved`;
        this.#sendLogMessage(`${this.#getTime()} - ${playerList} ${this.#getForcedString(forced)}${verb} to ${destination.channel}`);
    }
